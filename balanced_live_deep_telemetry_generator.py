...
print("=" * 80)
print("  AZTG-MC: Deep Multi-Cloud Telemetry Harvester (Perfectly Balanced 33.3% Split)")
print("=" * 80)
...
# ----------------- 1. LIVE AZURE HARVESTING (EQUALIZED SAMPLES) -----------------
print("\n[+] Ingesting Deep Metrics from Live Azure Infrastructure...")
for sp in azure_sps:
    if not sp["id"] or not sp["sec"]:
        continue
    c = sp["class"]
    # EQUAL BURST: Fixed 4 samples per identity so all classes have equal representation
    burst_size = 4
...
print(f"    ✅ Ingested {len(harvested_records)} deep Azure records (Equally distributed).")

# ----------------- 2. LIVE AWS HARVESTING (EQUALIZED SAMPLES) -----------------
...
for u in aws_users:
    ...
    if not ak_id or not ak_sec:
        continue
    c = u["class"]
    # EQUAL BURST: Fixed 4 samples per identity
    burst_size = 4
...
# ----------------- 3. SYNTHESIZE BALANCED 28-DAY CORPUS -----------------
print("\n[+] Synthesizing 28-Day Balanced Longitudinal Corpus...")
full_corpus = []
...
print("\n[+] Confirmed Class Distribution (Exact Balanced Split):")
counts = df["target_class"].value_counts().sort_index()
for c, count in counts.items():
    name = "Reader (Class 0)" if c==0 else ("Contributor (Class 1)" if c==1 else "Owner (Class 2)")
    print(f"   • {name:<24} : {count:,} records ({count/len(df)*100:.2f}%)")
print("=" * 80)
